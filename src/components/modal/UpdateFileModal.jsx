...
import { updateImgOrDocs } from '../utils/Api';
import Cookies from 'js-cookie';

const UpdateFileModal = ({ showModal, setShowModal, itemToEdit, onSuccess }) => {
  ...
  useEffect(() => {
    if (itemToEdit) {
      setFormData({
        ...
        productCompany: itemToEdit.productCompany || 'JIFSA',
        image: null // Don't set the existing file, as we only update if a new file is selected
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateImgOrDocs(itemToEdit._id, formDataToSend);
