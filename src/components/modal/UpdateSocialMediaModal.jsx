...
import { updateSocialMediaPost } from '../utils/Api';

const UpdateSocialMediaModal = ({ showModal, setShowModal, itemToEdit, onSuccess }) => {
  ...
  // Update form data when itemToEdit changes
  useEffect(() => {
    if (itemToEdit) {
      setFormData({
        ...
        uploadType: itemToEdit.uploadType || '',
        date: itemToEdit.date ? new Date(itemToEdit.date).toISOString().split('T')[0] : '',
        source: null,
        sourceUrl: itemToEdit.uploadType === 'flyer' ? itemToEdit.flyerUrl || itemToEdit.sourceUrl || '' : itemToEdit.sourceUrl || ''
      });
      setUploadedFileName(''); // Reset file name when editing a new item
    }
  }, [itemToEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateSocialMediaPost(itemToEdit._id, formDataToSend);
      
      setShowModal(false);
      onSuccess && onSuccess();
    } catch (error) {
      console.error('Error updating social media data:', error);
      alert('Error updating social media data. Please try again.');
    } finally {
