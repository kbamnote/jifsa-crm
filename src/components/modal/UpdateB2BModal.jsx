...
import { updateB2B } from "../utils/Api";

const UpdateB2BModal = ({ showModal, setShowModal, selectedRecord, onSuccess }) => {
  ...
  useEffect(() => {
    if (selectedRecord) {
      setFormData({
        instituteName: selectedRecord.instituteName || "",
        clientName: selectedRecord.clientName || "",
        phoneNo: selectedRecord.phoneNo || "",
        instituteEmail: selectedRecord.instituteEmail || "",
        status: selectedRecord.status || "", // Changed from "active" to ""
        location: selectedRecord.location || "",
        visitingDate: selectedRecord.visitingDate ? selectedRecord.visitingDate.split('T')[0] : "",
        visitingTime: selectedRecord.visitingTime || "",
        image: null
      });
    }
  }, [selectedRecord]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateB2B(selectedRecord._id, submitData);
      
      onSuccess();
    } catch (err) {
      console.error("Error updating B2B:", err);
      setError(err.response?.data?.message || "Failed to update B2B record. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 backdrop:blur-lg bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 flex justify-between items-center">
          <h2 className="text-2xl font-bold text-white">Update B2B Record</h2>
          <button
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto max-h-[calc(90vh-180px)] p-6">
          ...
          <div className="mb-6">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              />
              <label htmlFor="image-upload-update" className="cursor-pointer">
        ...
        <div className="border-t border-gray-200 px-6 py-4 bg-gray-50 flex justify-end space-x-3">
          ...
          >
            {loading ? "Updating..." : "Update B2B Record"}
          </button>
        </div>
      </div>
    </div>
